...
| `src/lib/map/` | MapLibre integration (data-store, map-store, layers, sources, popups, APRS icons, right-click `map-context-menu.svelte` — Copy GPS / Copy grid / Add fixed beacon here / Add fixed point here; "Add fixed beacon" deep-links into `Beacons.svelte` via `#/beacons?lat=…&lon=…`. "Add fixed point" opens `fixed-point-dialog.svelte` (name + APRS `SymbolPicker`) and drops a client-side landmark into `fixed-points-store.svelte.js` (localStorage-persisted), rendered by `layers/fixed-points.js`; clicking a point opens a delete popup) |
| NEXRAD radar overlay | `src/lib/map/layers/radar.js` (imperative MapLibre layer) consumes a descriptor from `src/lib/map/sources/radar-source.js`. The source module is the **backend seam**: `ACTIVE_RADAR_BACKEND` selects `raster` (IEM N0Q national composite, v1) or `vector` (GRA-48 smoothed contour MVT). Flipping that one constant swaps backends — the layer module and `LiveMapV2.svelte` are backend-agnostic. Toggle + opacity persisted to `gw_radar_*` localStorage keys. Pure logic is unit-tested in `radar-source.test.js`. Tiles are served by the graywolf-maps origin Worker under `https://maps.nw5w.com/radar/*` (raster = edge-cached IEM pull-through; `.pbf` = GRA-48 contours from R2), riding the same `?t=` bearer token as the basemap. |
