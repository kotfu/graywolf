...
| Rust modem (cross armv6) | `cross` per [`../../Cross.toml`](../../Cross.toml), multi-stage custom image [`../../packaging/docker/Dockerfile.armv6-cross`](../../packaging/docker/Dockerfile.armv6-cross): stage 1 is a `linux/arm/v6` Raspbian rootfs (real ARMv6), stage 2 splices Raspbian's `crtbegin.o`/`crtend.o`/`libgcc.a` over Debian's ARMv7 cross-gcc runtime so the linker doesn't promote `.ARM.attributes` to v7 | `target/arm-unknown-linux-gnueabihf/release/graywolf-modem` (with `Tag_CPU_arch: v6`, `Tag_FP_arch: VFPv2`; verify with `arm-linux-gnueabihf-readelf -A`) | Release CI; stage 1 needs `docker/setup-qemu-action` in the workflow |
...
| Pre-built rust-bin (CI) | rust-build matrix in `.github/workflows/release.yml` | Per-target `cargo build --release` | `rust-bin/<os>_<arch>/graywolf-modem[.exe]` (linux_armv6 disambiguates from a future linux_armv7), plus top-level `graywolf-modem-{amd64,arm64}` for the docker context | Tag push |
